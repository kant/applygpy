...
class BGPLVM_PANAMA(BayesianGPLVMMiniBatch):
    ...
    def __init__(self, Y, input_dim, fixed_covariates, fixed_cov_kernels, X=None, X_variance=None, init='PCA', num_inducing=10,
        ...
        if not fixed_covariates is None:
        ...
        self._cov_kernels = fixed_cov_kernels
        self._fixed_covariates = fixed_covariates
        for i,[name, p] in zip(range(4, 4+len(self._fixed_covariates)), self._cov_kernels.iteritems()):
            self.link_parameter(Parameterized(name, [p]), i)
        self.covs_kernels = dict([(name,
                                   [ObsAr(self._fixed_covariates[name]), self._cov_kernels[name]])
                                  for name in self._cov_kernels.iterkeys()])
